...
class Drawer extends Backbone.View
	initialize: ->
		...
		_this = @
		#$(document).on "projectSuccessFileadded", "#projects", (e, filename, response) ->
		#	log "projectSuccessFileadded message managed by the DOM document element"
		#	log filename
		#	@modal = new ApplicationModal()
		#	
		#	_this.addGroupWithProject _this.addDataSource, filename
		#	return

		# NEESH: binds objects to elements in dom
		@$el.children("div").each (i,group) ->
			new Group
				el: $(group)
	...
	addDataSource: (projectId, filename) ->
		console.log("adding datasource to the project")
		r = jsRoutes.controllers.Projects.createDataSource()
		filename = filename.split(".",1).pop() + ".hdt"
		$.ajax
			type: r.method
			url: r.url
	...
	addGroupWithProject: (callback, filename) ->
		console.log("in group")
		r = jsRoutes.controllers.Projects.addGroup()
		$.ajax
			url: r.url
			...
			success: (data) ->
				...
				# expand the panel and then crete the project
				$('div[data-group="New group"] a.toggle').click()

				_view.createProject(callback, filename)
				#_view.$el.find(".groupName").editInPlace("edit")
...
class Group extends Backbone.View
	...
	createProject: (callback, filename) ->
		...
		$.ajax
			...
			success: (tpl) ->
				...
				if callback
					projectId = parseInt(_view.$el.attr("data-project"))
	...
	renameGroup: (name) =>
		if (name.trim() == "New group")
			return alert "Please select a valid group name"
...
class Project extends Backbone.View
	...
	renameProject: (name) ->
		if (name.trim() == "New project")
			return alert "Please select a valid project name"
		...
		$.ajax
			...
			success: (data) ->
				...
				# check if the group has the default name and force the user
				# to change it
				$('div[data-group="New group"] .groupName').editInPlace("edit")
    ...
    $.ajax
      ...
      success: (tpl) ->
        newFolder = new TaskFolder
          el: $(tpl).insertBefore(".tasks")
          project: @project
        newFolder.$el.find(".panel-heading > div").editInPlace("edit")
      error: (err) ->
        $.error("Error: " + err)
    false
