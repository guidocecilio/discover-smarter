...
case class Task(id: Pk[Long], folder: String, project: Long, title: String, done: Boolean, dueDate: Option[Date], eventType: String, assignedTo: Option[String])
...
object Task {
  ...
  val simple = {
    ...
    get[String]("task.event_type") ~
    get[Option[String]]("task.assigned_to") map {
      case id~folder~project~title~done~dueDate~eventType~assignedTo => Task(
        id, folder, project, title, done, dueDate, eventType, assignedTo
      )
    }
  }
      ...
      SQL(
        """
          insert into task values (
            {id}, {title}, {done}, {dueDate}, {assignedTo}, {project}, {folder}, {eventType} 
          )
        """
      ).on(
        'id -> id,
