...
object Tasks extends Controller with Secured {
  ...
  def index(project: Long) = IsMemberOf(project) { _ => implicit request =>
    Project.findById(project).map { p =>
      ...
      Ok(html.projectpage(p, tasks, team))
    }.getOrElse(NotFound)
  }
      ...
      errors => BadRequest,
      {
        case (title, dueDate, assignedTo) => 
          val task =  Task.create(
            Task(NotAssigned, folder, project, title, false, dueDate, "action", assignedTo)
          )
          Ok(html.items.eventitem(task, Project.findById(project).get.name))
      }
    )
  }
  ...
  def addFolder = Action {
    Ok(html.panels.eventsfolder("New folder"))
  }
